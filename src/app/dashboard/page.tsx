...
interface PurchaseOrderData {
  no: number;
  name: string;
  januari: number; // BigInt comes as string from API and is converted to number
  februari: number;
  maret: number;
  april: number;
  mei: number;
...
// Helper function to safely parse values as numbers (converted from BigInt)
const safeParseNumber = (value: string): number => {
  if (!value) return 0;
  try {
    // Parse as float and round to integer since we're using BigInt in the database
...
function DashboardContent() {
  ...
  const fetchPurchaseOrders = async () => {
    try {
      ...
      const transformedData = result.map((item: any, index: number) => ({
        ...
        no: index + 1,
        name: item.name,
        januari: (typeof item.januari === 'string' ? Number(item.januari) : item.januari) || 0,
        februari: (typeof item.februari === 'string' ? Number(item.februari) : item.februari) || 0,
        maret: (typeof item.maret === 'string' ? Number(item.maret) : item.maret) || 0,
        april: (typeof item.april === 'string' ? Number(item.april) : item.april) || 0,
        mei: (typeof item.mei === 'string' ? Number(item.mei) : item.mei) || 0,
        juni: (typeof item.juni === 'string' ? Number(item.juni) : item.juni) || 0,
        juli: (typeof item.juli === 'string' ? Number(item.juli) : item.juli) || 0,
        agustus: (typeof item.agustus === 'string' ? Number(item.agustus) : item.agustus) || 0,
        september: (typeof item.september === 'string' ? Number(item.september) : item.september) || 0,
        oktober: (typeof item.oktober === 'string' ? Number(item.oktober) : item.oktober) || 0,
        november: (typeof item.november === 'string' ? Number(item.november) : item.november) || 0,
        desember: (typeof item.desember === 'string' ? Number(item.desember) : item.desember) || 0,
        totalQtyPO: item.totalQtyPO || 0,
        totalValueSales: (typeof item.totalValueSales === 'string' ? Number(item.totalValueSales) : item.totalValueSales) || 0,
        group: item.groupName,
        targetGroup: item.targetGroup ? (typeof item.targetGroup === 'string' ? Number(item.targetGroup) : item.targetGroup) : 0,
        achieve: item.achieve ? (typeof item.achieve === 'string' ? Number(item.achieve) : item.achieve) : 0
      }));
  ...
  const handleCellSave = async () => {
    ...
    if (editingCell) {
      ...
      if (editingCell.col === 'name') {
        ...
        const numericValue = safeParseNumber(editValue) || 0;
      ...
      // Save to database
        try {
          ...
          const purchaseOrder = {
            ...
            name: rowData.name,
            group: rowData.group,
            januari: Number(rowData.januari),
            februari: Number(rowData.februari),
            maret: Number(rowData.maret),
            april: Number(rowData.april),
            mei: Number(rowData.mei),
            juni: Number(rowData.juni),
            juli: Number(rowData.juli),
            agustus: Number(rowData.agustus),
            september: Number(rowData.september),
            oktober: Number(rowData.oktober),
            november: Number(rowData.november),
            desember: Number(rowData.desember),
            totalQtyPO: Number(rowData.totalQtyPO),
            totalValueSales: Number(rowData.totalValueSales),
            targetGroup: rowData.targetGroup ? Number(rowData.targetGroup) : null,
            achieve: rowData.achieve ? Number(rowData.achieve) : null
          };
  ...
  const handleTargetSave = async () => {
    if (editingTarget) {
      const newTarget = safeParseNumber(targetEditValue) || 0;
      ...
      try {
        ...
        for (const member of groupMembers) {
          ...
          const purchaseOrder = {
            ...
            name: member.name,
            group: member.group,
            januari: Number(member.januari),
            februari: Number(member.februari),
            maret: Number(member.maret),
            april: Number(member.april),
            mei: Number(member.mei),
            juni: Number(member.juni),
            juli: Number(member.juli),
            agustus: Number(member.agustus),
            september: Number(member.september),
            oktober: Number(member.oktober),
            november: Number(member.november),
            desember: Number(member.desember),
            totalQtyPO: Number(member.totalQtyPO),
            totalValueSales: Number(member.totalValueSales),
            targetGroup: Number(newTarget)
          };
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(data.map(item => ({
      ...
      'Achievement %': item.targetGroup ? ((Number(item.totalValueSales) / Number(item.targetGroup)) * 100).toFixed(2) : 0
    })));
...
const drawGroupPerformance = (): void => {
  ...
  groups.forEach((group: string, index: number) => {
    ...
    const achievement = groupTarget ? ((Number(groupTotal) / Number(groupTarget)) * 100) : 0;
...
const getGroupPerformanceData = (): GroupData[] => {
  ...
  data.forEach(item => {
    ...
    if (groupTotals.hasOwnProperty(group)) {
      groupTotals[group] = Number(groupTotals[group]) + Number(item.totalValueSales);
    }
  });
  
  const totalSales = Object.values(groupTotals).reduce((sum, val) => sum + Number(val), 0);
...
const drawDataTable = (): void => {
  ...
  const getGroupTotals = (group: string) => {
    ...
    return groupData.reduce((sum, item) => sum + Number(item.totalValueSales), 0);
  };

  const getTotalSales = () => {
    return data.reduce((sum, item) => sum + Number(item.totalValueSales), 0);
  };

  const getGroupAchievement = (group: string) => {
    const groupData = data.filter(item => item.group === group);
    const totalGroupSales = groupData.reduce((sum, item) => sum + Number(item.totalValueSales), 0);
